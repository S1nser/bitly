...
function buildOfferMessage(application, offer) {
  ...
  const lines = [
    lang.offerHeader,
    '',
    `⭐ *${escapeMd(offer.partner_name)}*`,
    offer.geo ? `🌍 ${lang.geoLabel}: ${escapeMd(offer.geo)}` : null,
    offer.rate ? `💰 ${lang.rateLabel}: ${escapeMd(offer.rate)}` : null,
    tagsLine ? `\n${escapeMd(tagsLine)}` : null
  ].filter(Boolean);
  return lines.join('\n');
}
...
function createBot() {
  ...
  bot.start(async (ctx) => {
    ...
    if (!payload) {
      return ctx.reply(t(lang).startNoToken);
    }

    const application = await db.getApplicationByToken(payload);
    if (!application) {
      return ctx.reply(t(lang).startTokenNotFound);
    }
  ...
  bot.action(/^offer:(\d+)$/, async (ctx) => {
    ...
    if (ADMIN_CHAT_ID) {
      ...
      const details = [
        ...
        `Оффер: ${offer.partner_name} (${offer.geo || '—'}, ${offer.rate || '—'})`,
        `Telegram: ${contact}`,
        `GEO трафика: ${app.geo || '—'}`,
...
module.exports = { createBot, sendOfferToApplicant };

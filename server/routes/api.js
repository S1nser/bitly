...
router.get('/applications/status/:token', async (req, res) => {
  try {
    const application = await db.getApplicationByToken(req.params.token);
    if (!application) {
      return res.status(404).json({ error: 'Заявка не найдена.' });
    }
    return res.json({ connected: Boolean(application.telegram_chat_id) });
  } catch (e) {
    console.error('[api] Не удалось проверить статус заявки:', e);

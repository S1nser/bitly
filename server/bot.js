...
const STR = {
  ru: {
    ...
    crClickRegLabel: '📊 CR клик → рег',
    crRegDepLabel: '💳 CR рег → деп',
    epcLabel: '💵 EPC (заработок за клик)',
  ...
  en: {
    ...
    crClickRegLabel: '📊 CR click → reg',
    crRegDepLabel: '💳 CR reg → dep',
  ...
  uz: {
    ...
    crClickRegLabel: "📊 CR klik → reg",
    crRegDepLabel: "💳 CR reg → dep",
    epcLabel: "💵 EPC (bir klik uchun daromad)",
    adminFeeLabel: '🏷 Admin fee',
  ...
  ar: {
    ...
    crClickRegLabel: '📊 CR كليك ← تسجيل',
    crRegDepLabel: '💳 CR تسجيل ← إيداع',
    epcLabel: '💵 EPC (الربح لكل كليك)',
    adminFeeLabel: '🏷 Admin fee',
...
function buildOfferMessage(application, offer) {
  ...
  const lines = [
    ...
    offer.cr_click_reg ? `${lang.crClickRegLabel}: <b>${escapeHtml(offer.cr_click_reg)}</b>` : null,
    offer.cr_reg_dep ? `${lang.crRegDepLabel}: <b>${escapeHtml(offer.cr_reg_dep)}</b>` : null,
    offer.epc ? `${lang.epcLabel}: <b>${escapeHtml(offer.epc)}</b>` : null,
...
// Отправляет несколько офферов одному и тому же получателю подряд (пакетная
// отправка пресетов из админки) — с небольшой паузой между сообщениями,
// чтобы не упереться в лимит Telegram на сообщения в секунду в один чат.
// Одно неудачное сообщение не прерывает отправку остальных.
async function sendOffersToApplicant(bot, application, offers) {
  if (!bot) throw new Error('Бот не запущен (нет BOT_TOKEN)');
  if (!application.telegram_chat_id) {
    throw new Error('У этой заявки ещё не подключён Telegram');
  }
  ...
  for (const offer of offers) {
    try {
      ...
      results.push({ offerId: offer.id, ok: true });
    } catch (e) {
      results.push({ offerId: offer.id, ok: false, error: e.message });
      console.error(`[bot] Пакетная отправка: оффер #${offer.id} не ушёл:`, e.message);
    }
...
module.exports = { createBot, sendOfferToApplicant, sendOffersToApplicant, broadcastToApplicants };
